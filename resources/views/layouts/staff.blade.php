...
<head>
    ...
    <title>@yield('title', 'Staff Dashboard') - {{ config('app.name', 'LuxuryFly') }}</title>
    ...
    <div class="min-h-screen flex">
        ...
        <aside class="w-64 bg-black border-r border-amber-500/10 fixed h-full z-40 flex flex-col">
            <div class="p-6 border-b border-amber-500/10">
                <a href="{{ route('staff.dashboard') }}" class="font-luxury text-xl font-bold tracking-widest text-amber-500">
                    LUXURY<span class="text-white font-light">FLY</span>
                    <span class="block text-xs text-zinc-500 font-sans font-normal tracking-normal mt-1">Staff Panel</span>
                </a>
            </div>
            <nav class="flex-1 p-4 space-y-0.5 overflow-y-auto sidebar-scroll">
                <a href="{{ route('staff.dashboard') }}" class="nav-link flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm {{ request()->routeIs('staff.dashboard') ? 'active' : 'text-zinc-400 hover:text-white hover:bg-zinc-900/50' }} transition">
                    <i class="fas fa-chart-bar w-5 text-center text-sm"></i>
                    <span>Dashboard</span>
                </a>
                <div class="pt-6 pb-1.5">
                    <p class="text-[10px] uppercase tracking-[0.2em] text-zinc-600 px-4 font-semibold">Operations</p>
                </div>
                <a href="{{ route('staff.flights') }}" class="nav-link flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm {{ request()->routeIs('staff.flights') ? 'active' : 'text-zinc-400 hover:text-white hover:bg-zinc-900/50' }} transition">
                    <i class="fas fa-plane w-5 text-center text-sm"></i>
                    <span>Flight Monitoring</span>
                </a>
                <a href="{{ route('staff.manifest') }}" class="nav-link flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm {{ request()->routeIs('staff.manifest*') ? 'active' : 'text-zinc-400 hover:text-white hover:bg-zinc-900/50' }} transition">
        ...
        <div class="flex-1 ml-64 flex flex-col min-h-screen">
                ...
                <div class="flex items-center justify-between px-8 py-3.5">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-[10px] text-zinc-500 uppercase tracking-wider">Staff</p>
                        </div>
                    </div>
                </div>
            </header>
            ...
            <footer class="px-8 py-4 border-t border-zinc-800/50">
                <p class="text-xs text-zinc-600 text-center">
                    &copy; {{ date('Y') }} <span class="text-amber-500/70 font-luxury">LUXURYFLY</span> Staff Panel. All rights reserved.
                </p>
            </footer>
        </div>
    </div>
